...
class TimelineEntry(BaseModel):
    ...
    company: str | None = Field(default=None, description="Employer name, as reported by CandidateProfile.")
    position: str | None = Field(default=None, description="Job title, as reported by CandidateProfile.")
    start_year: int | None = Field(default=None, description="Parsed start year, if resolvable.")
    start_month: int | None = Field(default=None, description="Parsed start month (1-12), if resolvable.")
    end_year: int | None = Field(default=None, description="Parsed end year, if resolvable and not ongoing.")
    end_month: int | None = Field(default=None, description="Parsed end month (1-12), if resolvable and not ongoing.")
    is_current: bool = Field(default=False, description="True if this role's end date indicated ongoing employment (e.g. 'Present').")
    duration_months: int | None = Field(default=None, ge=0, description="Computed tenure in months, if resolvable.")
...
class ExperienceMetrics(BaseModel):
    ...
    average_tenure_months: float | None = Field(default=None, ge=0.0, description="Average tenure across roles with resolvable duration; None if no role has a resolvable duration.")
    longest_tenure_months: int | None = Field(default=None, ge=0, description="Longest single-role tenure in months, among roles with resolvable duration.")
    shortest_tenure_months: int | None = Field(default=None, ge=0, description="Shortest single-role tenure in months, among roles with resolvable duration.")
...
class EmploymentGap(BaseModel):
    ...
    after_company: str | None = Field(default=None, description="Company held immediately before the gap.")
    before_company: str | None = Field(default=None, description="Company held immediately after the gap.")
...
class StabilityAnalysis(BaseModel):
    ...
    average_tenure_months: float | None = Field(default=None, ge=0.0)
    job_change_count: int = Field(..., ge=0, description="Number of transitions between recorded roles.")
    longest_tenure_months: int | None = Field(default=None, ge=0)
